...
function CalendlyCTA() {
  return (
    <div style={{
      background: 'linear-gradient(135deg, #0a1002 0%, #121f04 100%)',
      border: '0.5px solid rgba(123,201,6,0.25)',
      borderRadius: 12,
      padding: '24px 28px',
      marginTop: 2,
      display: 'flex',
      ...
      <div>
        ...
        <div style={{fontSize: 15, fontWeight: 700, color: '#ffffff', marginBottom: 5, lineHeight: 1.3}}>
...
export default function Dashboard() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#f8faf3',fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
      ...
      {/* LOADING BANNER */}
      {isLoading && (
        <div style={{position:'fixed',top:52,left:0,right:0,zIndex:40,background:'#121f04',borderBottom:'2px solid #7BC906',padding:'14px 24px',display:'flex',alignItems:'center',gap:14,animation:'fadeIn 0.25s ease'}}>
          ...
          <div>
            <div style={{fontSize:13,fontWeight:700,color:'#ffffff'}}>{loadingState==='searching'?'searching current sources...':'synthesizing intelligence...'}</div>
            <div style={{fontFamily:"'Hedvig Letters Serif',serif",fontSize:11,color:'rgba(123,201,6,0.5)',marginTop:2}}>{loadingState==='searching'?'tavily is retrieving real-time results from across the web':'claude is synthesizing findings into a structured response'}</div>
          </div>
          <div style={{marginLeft:'auto',display:'flex',gap:4}}>
            {[0,1,2].map(i => <div key={i} style={{width:6,height:6,borderRadius:'50%',background:'#7BC906',animation:`pulse 1.4s ease-in-out ${i*0.2}s infinite`}}/>)}
          </div>
        </div>
      )}
      ...
      <nav style={{position:'sticky',top:0,zIndex:50,background:'rgba(0,0,0,0.97)',backdropFilter:'blur(10px)',borderBottom:'0.5px solid rgba(123,201,6,0.15)',height:'52px',display:'flex',alignItems:'center'}}>
        <div style={{width:'100%',maxWidth:1100,margin:'0 auto',padding:'0 24px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
